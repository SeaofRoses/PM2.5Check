...
import com.changdi.pm25check.R;
...
public class DashboardView3 extends View {
    ...
    private String mHeaderText = "pm2.5数据"; // 表头
    ...
    private void init() {
        ...
        mBgColors = new int[]{
                ContextCompat.getColor(getContext(), R.color.color_vbad),
                ContextCompat.getColor(getContext(), R.color.color_bad),
    ...
    private String calculateCreditDescription() {
        if (mCreditValue > 250) {
            return "pm2.5报表啦，凑合活着吧";
        } else if (mCreditValue > 150) {
            return "重度污染，减少出行";
        } else if (mCreditValue > 115) {
            return "中度污染,需要佩戴口罩";
        } else if (mCreditValue > 75) {
            return "轻度污染";
        }else if(mCreditValue > 35){
            return "空气质量良";
        }
        return "空气质量较好";
    }
     ...
     * 信用分对应信用描述
     */
    private int calculateBGColorWithValue(float value) {
        if (value > 250) {
            return mBgColors[0];
        }else if(value > 150){
            return mBgColors[1];
